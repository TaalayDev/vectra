...
List<int> compress(List<int> pixels, int colorBits) {
  final book = CodeBook(colorBits);
  final buf = CodeBuffer(book)..add(book.clearCode);
  ...
  int code = pixels[0];
  for (final px in pixels.sublist(1)) {
...
class CodeBook {
  ...
  CodeBook(int initialColorBits) : colorBits = initialColorBits < 2 ? 2 : initialColorBits {
    assert(colorBits <= 8);
  ...
  /// Returns the new code after appending a pixel to the pixel string represented
  /// by the previous code, or null if the code isn't in the table.
  int? codeAfterAppend(int code, int pixelIndex) => _codeAfterAppend[(code << 8) | pixelIndex];
...
class CodeBuffer {
  ...
  List<int> finish() {
    ...
    // The final result starts with the number of color bits.
    final dest = <int>[book.colorBits];
