...
class VecDocumentState extends _$VecDocumentState {
  ...
  VecDocument _withLayer(String sceneId, String layerId, VecLayer Function(VecLayer) fn) {
  ...
  Future<void> save() async {
    ...
    final saved = state.copyWith(meta: state.meta.copyWith(modifiedAt: now));
    state = saved;
  ...
  void removeScene(String sceneId) {
    _commit(state.copyWith(scenes: state.scenes.where((s) => s.id != sceneId).toList()));
  }
    ...
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          layers: [
            ...scene.layers,
            VecLayer(id: layerId, name: name ?? 'Layer ${scene.layers.length + 1}', order: scene.layers.length),
          ],
  ...
  void updateLayer(String sceneId, String layerId, VecLayer Function(VecLayer) updater) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(tracks: scene.timeline.tracks.where((t) => t.layerId != layerId).toList()),
        );
      }),
    );
  }
      ...
      _withScene(sceneId, (scene) {
        ...
        final reordered = [
          for (var i = 0; i < orderedIds.length; i++)
            if (layerMap.containsKey(orderedIds[i])) layerMap[orderedIds[i]]!.copyWith(order: i),
        ];
      ...
      _withScene(sceneId, (scene) {
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(
            tracks: [
              ...scene.timeline.tracks,
              VecTrack(id: _uuid.v4(), layerId: layerId, shapeId: shape.id, keyframes: [kf]),
            ],
          ),
        );
      }),
  ...
  void addShapes(String sceneId, String layerId, List<VecShape> shapes) {
    _commit(
      _withScene(sceneId, (scene) {
        final newLayers = [
          for (final l in scene.layers)
            if (l.id == layerId) l.copyWith(shapes: [...l.shapes, ...shapes]) else l,
        ];
        return scene.copyWith(
          layers: newLayers,
          timeline: scene.timeline.copyWith(
    ...
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          layers: [
            for (final l in scene.layers)
              if (l.id == layerId) l.copyWith(shapes: l.shapes.where((s) => !shapeIds.contains(s.id)).toList()) else l,
          ],
          timeline: scene.timeline.copyWith(
            tracks: scene.timeline.tracks.where((t) => t.shapeId == null || !shapeIds.contains(t.shapeId)).toList(),
          ),
        );
      }),
    );
  ...
  void replaceShapes(String sceneId, String layerId, List<String> removeIds, List<VecShape> newShapes) {
    _commit(
      _withScene(sceneId, (scene) {
          ...
          if (removeIds.contains(s.id)) {
            ...
            newShapeList.addAll(
              newShapes.where((ns) {
                ...
                return ns.data.name?.startsWith(s.data.name ?? s.id) ?? false || ns.id.startsWith(s.id.substring(0, 8));
              }),
            );
          } else {
            newShapeList.add(s);
        ...
        final newTracks = [
          ...scene.timeline.tracks.where((t) => t.shapeId == null || !removeIds.contains(t.shapeId)),
  ...
  String groupShapes(String sceneId, String layerId, List<String> shapeIds) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
            ...
            .map(
              (s) => s.copyWith(
                data: s.data.copyWith(
                  transform: s.transform.copyWith(x: s.transform.x - minX, y: s.transform.y - minY),
                ),
              ),
            )
            .toList();
        ...
        final remaining = layer.shapes.where((s) => !shapeIds.contains(s.id)).toList();
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(tracks: [...cleanTracks, _trackAt0(layerId, group)]),
        );
      }),
    );
    return groupId;
  ...
  List<String> ungroupShape(String sceneId, String layerId, String groupId) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
        ...
        final groupShape = layer.shapes[groupIdx].maybeMap(group: (g) => g, orElse: () => null);
        ...
        final children = groupShape.children.map((c) {
          ...
          return c.copyWith(
            data: c.data.copyWith(
              transform: c.transform.copyWith(x: c.transform.x + gt.x, y: c.transform.y + gt.y),
            ),
          );
        }).toList();
        ...
        final cleanTracks = scene.timeline.tracks.where((t) => t.shapeId != groupId).toList();
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(
            tracks: [...cleanTracks, for (final child in children) _trackAt0(layerId, child)],
          ),
        );
      }),
    );
  ...
  void commitCurrentState() {
    ...
    if (prevDoc != null) {
      for (final newScene in state.scenes) {
        final oldScene = prevDoc.scenes.where((s) => s.id == newScene.id).firstOrNull;
        if (oldScene == null) continue;
        for (final newLayer in newScene.layers) {
          final oldLayer = oldScene.layers.where((l) => l.id == newLayer.id).firstOrNull;
          if (oldLayer == null) continue;
          for (final newShape in newLayer.shapes) {
            final oldShape = oldLayer.shapes.where((s) => s.id == newShape.id).firstOrNull;
            if (oldShape == null || oldShape == newShape) continue;
            finalDoc = _recordShapeToKeyframe(finalDoc, newScene.id, newLayer.id, newShape.id, selectedFrame);
          }
        }
      }
    }
  ...
  List<String> applyPathfinder(String sceneId, String layerId, List<String> shapeIds, PathfinderOp op) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
        ...
        final inputs = layer.shapes.where((s) => shapeIds.contains(s.id)).toList();

        final insertIdx = layer.shapes.indexWhere((s) => shapeIds.contains(s.id));
        final remaining = layer.shapes.where((s) => !shapeIds.contains(s.id)).toList();
        ...
        final newShapes = [...remaining.take(insertIdx), ...results, ...remaining.skip(insertIdx)];
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(
            tracks: [...cleanTracks, for (final shape in results) _trackAt0(layerId, shape)],
          ),
        );
      }),
    );
  ...
  String? expandCompound(String sceneId, String layerId, String shapeId) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
        ...
        final compound = layer.shapes[idx].maybeMap(compound: (c) => c, orElse: () => null);
        ...
        final cleanTracks = scene.timeline.tracks.where((t) => t.shapeId != shapeId).toList();
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(tracks: [...cleanTracks, _trackAt0(layerId, expanded)]),
        );
      }),
    );
  ...
  void reorderShape(String sceneId, String layerId, int fromIndex, int toIndex) {
    _commit(
      _withLayer(sceneId, layerId, (layer) {
  ...
  void _commitZOrder(String sceneId, String layerId, String shapeId, List<VecShape> Function(List<VecShape>, int) fn) {
    _commit(
      _withLayer(sceneId, layerId, (layer) {
        final idx = layer.shapes.indexWhere((s) => s.id == shapeId);
    ...
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          layers: [
            for (final l in scene.layers)
              if (l.id == layerId) l.copyWith(shapes: l.shapes.where((s) => s.id != shapeId).toList()) else l,
          ],
          timeline: scene.timeline.copyWith(tracks: scene.timeline.tracks.where((t) => t.shapeId != shapeId).toList()),
        );
      }),
    );
  }
  ...
  void removeSymbol(String symbolId) {
    _commit(state.copyWith(symbols: state.symbols.where((s) => s.id != symbolId).toList()));
  }
  ...
  String? convertToSymbol(String sceneId, String layerId, List<String> shapeIds, String name) {
    ...
    final symbol = VecSymbol(
      ...
      layers: [VecLayer(id: symbolLayerId, name: 'Layer 1', shapes: symbolShapes)],
    ...
    _commit(
      _withScene(sceneId, (scene) {
        ...
        return scene.copyWith(
          ...
          timeline: scene.timeline.copyWith(tracks: [...cleanTracks, _trackAt0(layerId, instance)]),
        );
  ...
  void removeSymbolLayer(String symbolId, String layerId) {
    updateSymbol(symbolId, (sym) => sym.copyWith(layers: sym.layers.where((l) => l.id != layerId).toList()));
  }
  ...
  void reorderSymbolLayers(String symbolId, List<String> orderedIds) {
    updateSymbol(symbolId, (sym) {
      ...
      return sym.copyWith(
        layers: [
          for (var i = 0; i < orderedIds.length; i++)
            if (map.containsKey(orderedIds[i])) map[orderedIds[i]]!.copyWith(order: i),
        ],
      );
    });
  }
  ...
  void updateSymbolShape(String symbolId, String layerId, String shapeId, VecShape Function(VecShape) updater) {
    updateSymbol(
      symbolId,
      (sym) => sym.copyWith(
        layers: [
          ...
          timeline: scene.timeline.copyWith(
            tracks: [
              for (final t in scene.timeline.tracks)
                if (t.id == trackId) t.copyWith(keyframes: [...t.keyframes, keyframe]) else t,
            ],
          ),
        );
      }),
  ...
  void updateKeyframe(String sceneId, String trackId, int frame, VecKeyframe Function(VecKeyframe) updater) {
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          timeline: scene.timeline.copyWith(
  ...
  void addKeyframeForShape(String sceneId, String layerId, String shapeId, VecKeyframe keyframe) {
    _commit(
      _withScene(sceneId, (scene) {
        final existing = scene.timeline.tracks.where((t) => t.layerId == layerId && t.shapeId == shapeId).firstOrNull;
        ...
        if (existing != null) {
          ...
          newTracks = [
            ...scene.timeline.tracks,
            VecTrack(id: _uuid.v4(), layerId: layerId, shapeId: shapeId, keyframes: [keyframe]),
          ];
        }
        ...
        return scene.copyWith(
          timeline: scene.timeline.copyWith(
            tracks: [
              for (final t in scene.timeline.tracks)
                if (t.layerId == layerId && t.shapeId == shapeId)
                  t.copyWith(keyframes: t.keyframes.where((k) => k.frame != frame).toList())
                else
                  t,
            ],
          ),
  ...
  void moveKeyframeForShape(String sceneId, String layerId, String shapeId, int fromFrame, int toFrame) {
    ...
    _commit(
      _withScene(sceneId, (scene) {
        ...
        final newTracks = scene.timeline.tracks.map((t) {
          ...
          final kfToMove = t.keyframes.where((k) => k.frame == fromFrame).firstOrNull;
          if (kfToMove == null) return t;
          final others = t.keyframes.where((k) => k.frame != fromFrame && k.frame != clampedTo).toList();
          final moved = [...others, kfToMove.copyWith(frame: clampedTo)]..sort((a, b) => a.frame.compareTo(b.frame));
  ...
  void applyAnimationPreset(String sceneId, String layerId, String shapeId, AnimationPreset preset, int atFrame) {
    ...
    final existingTrack = scene.timeline.tracks.where((t) => t.layerId == layerId && t.shapeId == shapeId).firstOrNull;
    ...
    final anchoredShape = existingTrack == null
        ? baseShape
        : KeyframeInterpolator.applyAtFrame(baseShape, existingTrack.keyframes, atFrame);
    ...
    for (final pk in preset.keyframes) {
      ...
      final kf = presetKf.copyWith(frame: absoluteFrame, transform: resolvedT, tweenType: VecTweenType.motion);
  ...
  void setTimelineDuration(String sceneId, int duration) {
    ...
    _commit(_withScene(sceneId, (s) => s.copyWith(timeline: s.timeline.copyWith(duration: duration))));
  }

  void setLoopType(String sceneId, VecLoopType loopType) {
    _commit(_withScene(sceneId, (s) => s.copyWith(timeline: s.timeline.copyWith(loopType: loopType))));
  }
      ...
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          timeline: scene.timeline.copyWith(
            tracks: [
              for (final t in scene.timeline.tracks)
                if (t.id == trackId) t.copyWith(keyframes: t.keyframes.where((k) => k.frame != frame).toList()) else t,
            ],
          ),
        );
      }),
  ...
  void removeMotionPath(String sceneId, String motionPathId) {
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(motionPaths: scene.motionPaths.where((p) => p.id != motionPathId).toList());
      }),
    );
  }

  void updateMotionPath(String sceneId, String motionPathId, VecMotionPath Function(VecMotionPath) updater) {
    _commit(
      _withScene(sceneId, (scene) {
        return scene.copyWith(
          motionPaths: [
  ...
  void updateMotionPathNodesNoHistory(String sceneId, String motionPathId, List<VecPathNode> nodes) {
