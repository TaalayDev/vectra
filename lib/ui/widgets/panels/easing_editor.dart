...
/// Opens the easing editor with per-property tabs.
/// [onChanged] receives the full updated [VecKeyframe] on every change.
Future<void> showKeyframeEasingEditor({
  required BuildContext context,
  required AppTheme theme,
  required VecKeyframe keyframe,
  required void Function(VecKeyframe) onChanged,
}) {
...
class _EasingEditorDialog extends StatefulWidget {
  ...
  final void Function(VecKeyframe) onChanged;
...
class _EasingEditorDialogState extends State<_EasingEditorDialog> {
  late VecKeyframe _kf;
  _EasingProp _activeProp = _EasingProp.all;
  ...
  VecEasing? get _currentEasing => _activeProp.easingFrom(_kf);
  bool get _isCustom => _currentEasing is VecCubicBezierEasing;
  ...
  void _commit(VecEasing? easing) {
    setState(() {
      _kf = _activeProp.setEasing(_kf, easing);
      _syncHandles();
    });
    widget.onChanged(_kf);
  }

  void _selectPreset(VecEasing? easing) => _commit(easing);

  void _switchToCustom() {
    _commit(VecEasing.cubicBezier(x1: _x1, y1: _y1, x2: _x2, y2: _y2));
  }
  ...
  void _onPanEnd(DragEndDetails _) {
    if (_dragHandle != null) widget.onChanged(_kf);
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: theme.divider.withAlpha(80), width: 0.5),
                ),
              ...
              // ── Reset per-property override ──────────────────────
              if (_activeProp != _EasingProp.all && _activeProp.easingFrom(_kf) != null) ...[
...
class _PropTab extends StatelessWidget {
  ...
  final String label;
  final bool selected;
  final bool hasOverride; // true if per-property easing differs from "All"
  final AppTheme theme;
